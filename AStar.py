    ...
    for index in range(len(problem.distance[parent_index])):
        if problem.distance[parent_index][index] > 0:     # 현재 state로부터 연결되어 있다면
            ...
            if not IsVisitedState(state, parent_node):    # 이미 지나온 경로인지 확인
                ...
                node.f = node.path_cost + problem.straight_line_distance[index]     # A* Search
...
# fringe가 f(g+h)값을 기준으로 한 PriorityQueue와 같이 동작하는 경우 A* Search가 됨
def TreeSearch(problem, fringe):
    node = Node()
    node.state = problem.initial_state
    node.depth = 0
    node.f = 0 + problem.straight_line_distance[0]
    fringe.put((node.f, node))                # initial state로 만든 노드 추가, Breadth First Search와 다른 코드
    print("Expanded State : ", end="")

    while True:
        if fringe.empty():
            return False
        node = fringe.get()[1]               # Breadth First Search와 다른 코드
        print(f"{node.state}({node.f}) ", end="")
        if node.state == problem.goal_state:
            return node
        expanded_nodes = Expand(node, problem)
        for node in expanded_nodes:
            fringe.put((node.f, node))        # Breadth First Search와 다른 코드

# A* Search를 수행하기 위해 PriorityQueue를 사용하여 fringe를 만들고 TreeSearch 함수를 실행함
def main():
    problem = Problem()
    print("<<< A* Search >>>")
    fringe = PriorityQueue()     # 우선순위 Queue, Breadth First Search와 다른 코드
