...
# fringe가 Queue(FIFO-First In First Out)와 같이 동작하는 경우 Breadth First Search가 됨
def TreeSearch(problem, fringe):
    node = Node()
    node.state = problem.initial_state
    node.depth = 0
    fringe.put((node.path_cost, node))                # initial state로 만든 노드 추가
